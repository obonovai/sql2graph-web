...
// top run bar stays lean (just target + the Translate action).
export function ValidationSettingsForm() {
  const target = useStore((st) => st.form.target);
  const validation = useStore((st) => st.form.validation);
  const setValidationMode = useStore((st) => st.setValidationMode);
