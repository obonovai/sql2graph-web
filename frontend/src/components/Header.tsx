...
import { useStore, usesThrowawayDb } from "@/hooks/useStore";
...
export function Header() {
  ...
  const validationMode = useStore((s) => s.form.validation.mode);
  const throwaway = useStore((s) => usesThrowawayDb(s.form));
  ...
  return (
    <div className="flex items-center gap-3 border-b border-slate-200 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">sql2graph</span>
      </div>
      <div className="ml-auto flex items-center gap-2">
        <Chip tone="indigo" size="md" title="Active model">
          {provider} / {model}
        </Chip>
        <Chip tone="indigo" size="md" title="Validation mode">
          {validationLabel}
        </Chip>
